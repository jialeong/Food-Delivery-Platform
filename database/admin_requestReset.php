...
if (isset($_POST["email"])) {
    ...
    try {
        ...
        //Content
        $url = "http://" . $_SERVER["HTTP_HOST"] . dirname($_SERVER["PHP_SELF"]) . "/admin_resetPassword.php?code=$code";
        $back_loginPage = "login_page.php";
        $mail->isHTML(true);                                  //Set email format to HTML
        $mail->Subject = 'Admin your password reset link';
        $mail->Body    = "<h1>Admin you requested a password reset</h1>
                            Please <a href='$url'>click here</a> to reset password";
        $mail->AltBody = 'This is the body in plain text for non-HTML mail clients';

        $mail->send();
        echo "<h3>Admin reset password link has been sent to your email /
        <a href='$back_loginPage'>Back to login page</a></h3>";
    } catch (Exception $e) {
        echo "<h3>Admin message could not be sent. Mailer Error: {$mail->ErrorInfo}</h3>";
    }
    exit();
}
    ...
    <form method="POST" name="forget_password_Form" onsubmit="return(ValidateEmail());">
        ...
        <div class="Forgot_Passowrd_Container">
            ...
            <div class="Welcome_Statement">
                <p class="text-primary">Admin please enter your registered email address. &#128521;</p>
            </div>
